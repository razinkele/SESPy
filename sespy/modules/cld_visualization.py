...
def cld_viz_server(
    ...
    *,
    ...
    def _network():
        try:
            size_scale = safe_float(input.node_size_scale(), 1.0)
            font_scale = safe_float(input.font_size_scale(), 1.0)
            level_sep = safe_int(input.level_separation(), 90)
            node_sp = safe_int(input.node_spacing(), 200)
