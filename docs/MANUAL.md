...
**Controls.** "Remove these nodes" (multi-select), "Metric", "Reset (no ablation)". In "Intervention simulation": "Intervene at", "Steps", "Tokens", "Run simulation". In "Bayesian inference (path set)": "From", "To", "Also high" and "Also low" (extra evidence on elements of the causal paths), "Query" (forward: the source is high; diagnostic: the target is high), "Run inference".
