...
**Not yet validated (the larger corpus):** the companion `mosaicses-design.md` and `scientific-basis.md` contain ~60+ further citations (connectivity theory, telecoupling, Curonian biogeochemistry, ~14 AphiaIDs, and SP4's Anthropic-SDK claims). The full list to validate is in Part B. The 15 I checked are the foundational/central ones and all passed — a strong signal, but a complete pass over the remaining ~45 (esp. the AphiaID→species resolutions via the ICES/WoRMS MCP, and the no-DOI items) is recommended as a follow-up.
