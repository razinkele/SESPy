...
# Authoritative scientific-caveat text for this worked example (spec §7). NOT rendered
# in the live Studio this phase: the interactive editor builds ad-hoc scenarios and shows
# the generic structural-analysis disclaimer in scenario_view_ui(). Surfacing a *named*
# scenario's caveats (alongside the inert drift_banner) requires loading that scenario into
# the Studio, which is the deferred sidecar-load follow-on (review R1). Kept as the canonical
# caveat copy so the follow-on renders it verbatim.
CAVEATS = (
...
- **Spec §8 descope:** the side-by-side baseline/materialised composite-graph view (+ a11y graph table) AND the **sidecar load/save UI + functional baseline-drift banner + named-scenario caveat rendering** are deferred to a follow-on; the C2 `drift_banner` output ships as an inert placeholder, and the B1 `CAVEATS` constant is the canonical caveat copy the follow-on will render once a named scenario can be loaded into the Studio (nothing loads a sidecar to populate either this phase — review R1). C2 delivers the five diff tables + generic structural-analysis disclaimer + the "changed rows only" filter. (Spec §8/§14 trimmed to match.)
...
- **Verified by plan-review pass 4 — full integration dry-run** (workflow `wf_c733b5c5-f0c`; the entire plan applied verbatim at every real path on a throwaway branch `_review_dryrun4`, then discarded): `import app` OK; the **complete non-e2e suite ran 422 passed, 15 deselected, 0 failed — ZERO regressions** against the existing tree; all **25 new tests pass**, including `test_app_mounts_scenario_panel` (the C3 app-mount test passes 1–3 could not run because it needs the real app mounted). Seams re-checked against current source before transcription (`sespy.network.remove_nodes`, `sespy.data_structure.{Element,IsaData}`, `multises.channels.make_channel`, `replace_compartment`/`replace_channel`, the comparative/composite functions). Only 3 LOW findings — the `CAVEATS` overclaim (fixed above: comment corrected, folded into the R1 deferral), the documented inert `drift_banner`, and a latent mutable-`target`-on-frozen-`Intervention` caveat (no code hashes interventions; duplicate detection uses tuple keys). Repo restored clean (on `main`, HEAD `e816437`, empty status, branch deleted). **Plan is execution-ready.**
