...
The inspector (`topology_inspector_sb`, `topology.py:284-297`) selects a target via `inspector_target` and renders read-only detail in `inspector_detail` (`topology.py:376-411`). Add a separate `@render.ui` output **`inspector_tenet_editor`** (definitive id). **Mounting:** add `ui.output_ui("inspector_tenet_editor")` to the inspector sidebar in `topology_ui()`, directly **below** `inspector_detail`. The render function:

- **Resolves the live Channel by id** (the inspector dict `_inspector_node_info` omits `tenet_scores` and exposes a display-massaged `channel_type`): `target = input.inspector_target()`; `ch = next((c for c in ms.channels if c.id == target), None)`. Gate: render the editor only when `ch is not None and ch.channel_type == "governance"`. For a **non-governance channel that already carries `tenet_scores`** (possible via seed/JSON), render a short read-only note ("Tenet editing is available for governance channels only") so existing scores aren't silently un-editable; otherwise render nothing.
- **Renders all inputs inside this `@render.ui`** (never `ui.update_select` — that bleeds stale client values across selections). Each re-render reads `selected=` **from the channel data** (`ch.tenet_scores`), NOT from the prior input value — this is what guarantees fresh values on selection change (Shiny replaces the DOM subtree on re-render; reading from data, not from `input`, is load-bearing). The unit comprises: 10 `ui.input_select(f"tenet_{slug}", label, choices={"":"— not scored (gap)","1":"1",…,"5":"5"}, selected=str(ch.tenet_scores.get(slug,"")) )` in `TENETS` order (partial scoring OK per #19); a hidden **`channel_tenet_editing_id`** stamping `ch.id`, rendered as `ui.div(ui.input_text("channel_tenet_editing_id", "", value=ch.id), style="display:none")` (a real Shiny input widget registers with `input.<id>()` and echoes its server-set value on bind; the wrapping `div` hides the label+input — do NOT use a raw `<input type="hidden">`); and `ui.input_action_button("save_channel_tenets", "Save scores")`.
...
Add a `ui.nav_panel("Evaluative scores", ui.input_select("overlay_element", "Element", choices={}), ui.output_ui("overlay_editor"))` to the Compartments `navset_tab` in `compartments_ui()` (`compartments.py:77-89`), placed **immediately after the "Edit Data" panel**. Select-then-edit:

- **`overlay_element` is a static `ui.input_select`** declared in `compartments_ui()` (above), **repopulated by a `@reactive.effect` that calls `ui.update_select("overlay_element", choices=…, session=session)`** — analogous to the existing `_populate_picker` effect (`compartments.py:103-117`). The effect depends on `active_multises` + `active_compartment_id`; choices are the active compartment's eligible elements — Responses (`type == "Responses"`) and outcomes (`type ∈ OUTCOME_ELEMENT_TYPES`), labelled `"{label} ({type})"`, value = element id. **If `active_compartment_id` is `None`** (a compartment-less MultiSES) the effect sets empty choices and must not call `ms.compartment(None)`.
- A `@render.ui` output **`overlay_editor`** keyed on `input.overlay_element()`, rendering as one unit, each input reading `selected=` **from the element's overlay data** (not from prior input — load-bearing, see §3): the matching inputs (Response → 10 `ui.input_select(f"tenet_{slug}", label, choices={"":"— not scored (gap)","1":"1",…,"5":"5"}, selected=…)` pre-filled from `response_tenet_scores.get(eid, {})`; outcome → `ui.input_checkbox_group("equity_dims", choices={slug: label for slug, label in EQUITY_DIMENSIONS}, selected=outcome_equity_dimensions.get(eid, []))`); a hidden **`overlay_editing_id`** stamping the eid, rendered as `ui.div(ui.input_text("overlay_editing_id", "", value=eid), style="display:none")` (registering widget — never a raw `<input type="hidden">`); and `ui.input_action_button("save_overlay", "Save")`. The button lives inside `overlay_editor`, so `input.save_overlay` is live only while an editor is rendered — and `@reactive.event(input.save_overlay)` is silently suppressed (Shiny `SilentException`) when it isn't, so **no explicit `req()` guard is needed**.
...
- Topology: `inspector_tenet_editor` exists; **named positive** test (governance target → editor + `save_channel_tenets` + 10 `tenet_*` present); **named negative** test — drive the render with a non-governance target (use any non-governance channel from `seed_curonian()`, e.g. a `nutrients`/`water_discharge` channel) and a compartment target → `"save_channel_tenets" not in html`.
...
- Topology: select a governance channel, `wait_for_selector` for the editor container (e.g. `#topology-save_channel_tenets`), set `#topology-tenet_ecological` to "5", click Save, navigate to Comparative, `wait_for_selector("#comparative-tenet_table tbody tr")`. Cell-level value targeting inside a Shiny `DataGrid` has no established locator pattern in this repo, so assert at **row level** (a tenet-readiness row for the edited channel's subject exists) rather than a specific cell value — keep it consistent with the existing `test_comparative_e2e.py` row-presence idiom.
- Compartments: drill in, open "Evaluative scores", select an outcome element, `wait_for_selector` for `#compartments-equity_dims`, check `livelihood_displacement`, click Save, navigate to Comparative, assert the rendered `equity_table` contains the **display label** "Livelihood displacement" (the `equity_table` renderer maps the stored slug `livelihood_displacement` → label via `_labels`, so the visible cell text is the label, not the slug). Row-level / text-contains assertion, per the same idiom.
...
| `multises_app/modules/topology.py` | `topology_ui()`: mount `ui.output_ui("inspector_tenet_editor")` below `inspector_detail`. `topology_server`: `inspector_tenet_editor` `@render.ui` (governance-gated, direct Channel lookup, stamped id, read-only note for scored non-gov channels) + `save_channel_tenets` effect (save-time re-gate) + `replace_channel`; import `_ChannelValidationError` |
| `multises_app/modules/compartments.py` | `compartments_ui()`: new "Evaluative scores" nav panel (static `overlay_element` select + `ui.output_ui("overlay_editor")`) after "Edit Data". `compartments_server`: `@reactive.effect`+`ui.update_select` repopulating `overlay_element` (like `_populate_picker`); `overlay_editor` `@render.ui` (stamped `overlay_editing_id`, branch-before-read); `save_overlay` effect (save-time existence + `cid is None` guard) + `replace_compartment_overlays`; import `_ChannelValidationError`; `EQUITY_DIMENSIONS` already exported from `multises` (#20) |
...
## 10. Out-of-scope follow-ups (consciously deferred)
These were raised in a domain/UX review and are **deliberately deferred** — this increment's scope is "make the existing overlay fields editable," and each item below adds a new data field, a new app-wide surface, or a workflow capability beyond that. Recorded here so they're acknowledged, not silently missing.
- **Unsaved-change loss on tab close.** Per the chosen scope, persistence matches the existing app (in-memory + manual "Save (download .json)", no autosave). For the heavier scoring workload this raises a real data-loss risk; a cheap app-wide mitigation (a `window.onbeforeunload` "you have unsaved changes" warning driven by a dirty flag vs. last download) is a strong follow-up but needs app-wide dirty-state tracking not designed here. **Editing this increment does not add autosave.**
- **Score provenance & justification.** No per-score notes/justification, assessor ("who/when"), or confidence fields. These matter for research credibility (vs. demonstration), but each is a new overlay field on `Channel`/`Compartment` (additive, no schema bump) — a separate increment. Without them, scores are not attributable from the file alone.
- **"Reviewed — none apply" equity state.** The editor collapses a cleared equity selection to absent (§2.3); this loses the distinction between "explicitly reviewed, no dimensions apply" (an explicit `[]`, which the #20 validator permits) and "never reviewed" (absent). Analytically identical today; preserving the explicit `[]` is a possible refinement.
- Editable grid / bulk entry; undo-redo; confirmation modal on full clear; tenet scoring on non-governance channels (currently read-only-with-note when scores exist); weighting / composite indices.
