...
**Acceptance (§12-B)** is stated as concrete, computed **metric deltas** on the real seed — e.g. the restored wetland redistributes `leverage_hotspots` toward the new habitat, `response_pressure_gap` coverage changes for the breached Pressure, `inter_compartment_metrics` betweenness shifts with the new tidal channel — **bound to the exact element/channel ids the factory creates**, verified by running `compare_scenario` once and pinning the observed deltas.
