...
Each diff is an **outer join** on its key; added rows render `before=NaN`, removed rows `after=NaN`; `delta = after − before` for numeric cols, set-diff for list cols, carry-through (with a `changed` flag) for descriptive cols. The result is the literal `{before, after, delta}` shape `sespy.network.intervention_impact` already uses — but computed at the `MultiSES` level across all five metrics.

## 7. Depolderisation worked example (sub-project B)

`multises/scenarios/depolderisation.py` — `build_depolderisation_scenario(ms)` is a **structural factory** grounded in the *actual* Curonian seed (the reviews enumerated it; rev-2's assumptions were wrong):
1. `add_node` a polder/dyke Pressure + its incident State-suppression connections into `curonian_lagoon` — the pre-depolderisation barrier the generic seed does not encode;
2. `remove_node` that polder Pressure (the breach);
3. `add_node` a restored intertidal-wetland habitat (`type="Marine Processes & Functioning"`), a regulating buffering service (`type="Ecosystem Services"`), and its outcome (`type="Goods & Benefits"`) — **canonical type strings**, **wired with authored connections into the lagoon's existing chain** so the new ecosystem genuinely participates (not a disjoint island — *review DF-2*);
4. `add_channel` a restored tidal exchange **`curonian_lagoon → klaipeda_strait`** (its true adjacent estuary — **the lagoon does NOT border `baltic_se`** — *review T5/C2/DF-3*) as `organisms_marine_estuarine` (a `channel_type` not already present on that pair, so no `nx.DiGraph` parallel-edge collapse).

**Acceptance (§12-B)** is stated as concrete, computed **metric deltas** on the real seed, **bound to the exact element/channel ids the factory creates** and verified by running `compare_scenario` once and pinning the observed deltas. Prototyped on the seed, the substantive deltas are: the restored wetland adds to the lagoon `element_count` and redistributes `leverage_hotspots`/`compartment_summary.mean_leverage`; the polder breach changes `response_pressure_gap` for the affected Pressure. **The restored tidal channel registers ONLY in `inter_compartment_metrics`' `outgoing_channel_types` set-diff (a new `organisms_marine_estuarine` type on the lagoon→strait edge) — NOT in `channel_out_degree`/`betweenness`, because `curonian_lagoon`↔`klaipeda_strait` is already connected and the composite `nx.DiGraph` collapses the parallel edge** (confirmed: out-degree stays 2). The §12-B test pins these specific deltas, not aspirational topology shifts.
...
- **Intervention-editor sidebar** — pick kind → compartment → target → attributes → rationale; list + add/remove. **Reuse the real selectize editors in `modules/topology.py` (channel editor) / `modules/compartments.py` (element editor) for the input grammar** — `overlay_edit.py` is Shiny-free helpers only (`set_overlay_entry`, `friendly_error`), not UI grammar *(review T20)*.
- **Before/after comparison** — the five metric-diff `render.DataGrid`s (with a "changed rows only" filter), and a **side-by-side / toggle composite-graph view** (baseline vs materialised) reusing `output_pyvis_network`; nodes/edges added by the scenario are highlighted (new = ringed, removed = ghosted) — **no per-element direction tinting** (that's the deferred overlay), so the canvas just renders the two graphs, which `cross_view`'s path already supports.
- **`digraph_table_ui` a11y fallback**, the `sticky-disclaimer` stating the structural-analysis-not-prediction contract, and a **baseline-drift banner**: on load, count resolved vs dangling interventions and surface a staleness summary if the sidecar was authored against a different baseline *(review T22)*.
...
| Depolderisation | self-grounding factory; **wetland wired into the lagoon chain**; tidal channel to **`klaipeda_strait`** | the seed has no polder; the lagoon's marine neighbour is the strait, not the coastal sea |
...
- **B:** `build_depolderisation_scenario(ms)` end-to-end — materialises without error, the wetland is wired (non-isolated), the tidal channel lands on `klaipeda_strait`, and the **metric deltas match the pinned expected values** for the concrete created ids (no aspirational claims).
