...
**Caveats.** Caveat (1) — this is a **structural network analysis, not a process/biogeochemical model**; it shows how the *connectivity and graph metrics* change, not predicted ecological dynamics, magnitudes, or timescales — **is rendered in the live Studio** as the generic `sticky-disclaimer`. The two scenario-*specific* caveats — (2) "designing new ecosystems" ≠ restored-to-reference; (3) endogenic breach / exogenic dependency (driven by the `Intervention` metadata, not the materialised element — *review T19*) — are the canonical `CAVEATS` copy on the depolderisation factory; they are **NOT rendered this phase** (surfacing a *named* scenario's caveats requires loading it into the Studio, the deferred sidecar-load follow-on — §8), and will be rendered verbatim once that lands. All three get EG-domain-author (Tagliapietra) review.
...
- **Deferred to a follow-on refinement:** the **sidecar load/save UI + its baseline-drift banner** (count resolved vs dangling interventions on load — *review T22*), **named-scenario caveat rendering** (the `CAVEATS` copy on a loaded scenario, §7), and the side-by-side baseline/materialised composite-graph view + `digraph_table_ui` a11y fallback. All three are blocked by the same missing capability — loading a *named* scenario into the Studio. v1 authors a scenario in-session; the accessible `render.DataGrid` diffs are the v1 output, the generic `sticky-disclaimer` (caveat 1) renders, and the `drift_banner` output exists as an inert placeholder.
